...
exports.movieRepositories = void 0;
...
class MovieRepositories {
    ...
    async create(movieData) {
        ...
        const newMovie = await movie_model_1.MovieModel.create(movieData);
        return newMovie;
    }
    async findAllMoviesWithTheatres() {
        const result = await movie_model_1.MovieModel.aggregate([
            {
                $lookup: {
                    from: "theatres",
                    let: { movieId: "$_id" },
    ...
    async findById(movieId) {
        ...
        // console.log(movie);
        const movie = await movie_model_1.MovieModel.aggregate([
            {
                $match: { _id: new mongoose_1.Types.ObjectId(movieId) }
            },
            {
                $lookup: {
                    from: "theatres",
...
exports.movieRepositories = movieRepositories;
